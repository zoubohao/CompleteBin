...
class DeepMetaBinBaseModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_dim: int = 2048,
                 layers = 4) -> None:
        super(DeepMetaBinBaseModel, self).__init__()
        self.layers = layers
...
class DeepMetaBinModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_dim = 2048,
                 layers = 4) -> None:
        super(DeepMetaBinModel, self).__init__()
        self.device = device
        logger.info(f"--> Model hidden dim: {hidden_dim}, layers: {layers}, device: {device}.")
        self.cov_mean_model = MLP(num_bam_files, 256, hidden_dim, p=dropout).to(device)
        self.cov_var_model = MLP(num_bam_files, 256, hidden_dim, p=dropout).to(device)
        
        self.pretrain_model = DeepMetaBinBaseModel(kmer_dim, 0, split_parts_list, dropout, hidden_dim, layers).to(device)
        self.train_model = DeepMetaBinBaseModel(kmer_dim, 0, split_parts_list, dropout, hidden_dim, layers).to(device)

        # self.projector_simclr = MLP(hidden_dim, hidden_dim * 4, feature_dim, dropout).to(device)
        
        self.projector_simclr = nn.Sequential(
            MLP(hidden_dim, hidden_dim * 4, hidden_dim * 3, dropout),
            MLP(hidden_dim * 3, hidden_dim * 2, feature_dim, dropout)
        ).to(device)
    ...
    def forward(self, seq_tokens_inputs, mean_val, var_val):
        ...
        with torch.no_grad():
            seq_taxon_enc = self.pretrain_model(seq_tokens_inputs).unsqueeze(1)
        cov_mean_fea_enc = self.cov_mean_model(mean_val).unsqueeze(1)
        cov_var_fea_enc = self.cov_var_model(var_val).unsqueeze(1)
        all_info_seq = torch.cat([self.train_model.get_token_proj(seq_tokens_inputs), 
                                  seq_taxon_enc, cov_mean_fea_enc, cov_var_fea_enc], dim=1)  # b, l, c
        all_info_seq = self.train_model.get_feature_of_tokens(all_info_seq) # b, l, c
        all_info_seq = encode_seq2vec(all_info_seq)
